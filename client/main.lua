...
function Process(action, start, tick, finish)
    ...
    Action = action
	if Action.icon then
		if PSRCore.Shared.Items[tostring(Action.icon)] then
			local img = "nui://qb-inventory/html/" -- default qb-core inventory link
			if not string.find(PSRCore.Shared.Items[tostring(Action.icon)].image, "http") then -- 👀 Slipped in support for custom html links too
				if not string.find(PSRCore.Shared.Items[tostring(Action.icon)].image, "images/") then --search for if the icon images have /images in the listing
					img = img.."images/"
				end
				Action.icon = img..PSRCore.Shared.Items[tostring(Action.icon)].image
			end
		end
	end
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("PSRCore:Notify", "You are already doing something!", "error")
        end
    else
        TriggerEvent("PSRCore:Notify", "Cant do that action!", "error")
    end
end
